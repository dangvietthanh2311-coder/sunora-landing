...
(function () {
  ...
  /* ---------- 6. Quiz chọn áo 30 giây ---------- */
  /* Map kết quả: unisex -> S1002; nữ + che phủ tối đa -> S1004; nữ + gọn -> S1001.
     Câu 3 chỉ gợi ý thêm phụ kiện (xe máy -> găng S1006, đi bộ -> khẩu trang S1005). */
  var quizBox = document.getElementById("quiz-chon-ao");
  if (quizBox) {
    var qPicks = {};
    var qResult = document.getElementById("quizResult");
    var qPickEl = document.getElementById("quizPick");
    var qExtraEl = document.getElementById("quizExtra");
    var qGoBtn = document.getElementById("quizGo");
    var qResetBtn = document.getElementById("quizReset");
    var qSku = "";
    var qSpotTimer = null;

    function cardName(sku) {
      var card = document.querySelector('.product[data-sku="' + sku + '"]');
      return card ? card.dataset.name : sku;
    }
    function computeQuiz() {
      if (qPicks[0] == null || qPicks[1] == null || qPicks[2] == null) return;
      qSku = qPicks[0] === "unisex" ? "S1002" : (qPicks[1] === "kin" ? "S1004" : "S1001");
      qPickEl.textContent = cardName(qSku);
      var extra = "";
      if (qPicks[2] === "xemay") extra = "Đi xe máy mỗi ngày, thêm " + cardName("S1006") + " là kín trọn tay lái.";
      if (qPicks[2] === "bo") extra = "Hay đi bộ ngoài trời, thêm " + cardName("S1005") + " cho làn da mặt.";
      qExtraEl.textContent = extra;
      qExtraEl.hidden = !extra;
      qResult.hidden = false;
    }
    quizBox.querySelectorAll(".quiz__step").forEach(function (step) {
      var q = parseInt(step.getAttribute("data-q"), 10);
      step.querySelectorAll(".quiz__opt").forEach(function (opt) {
        opt.addEventListener("click", function () {
          step.querySelectorAll(".quiz__opt").forEach(function (o) {
            o.classList.remove("is-picked");
            o.setAttribute("aria-pressed", "false");
          });
          opt.classList.add("is-picked");
          opt.setAttribute("aria-pressed", "true");
          qPicks[q] = opt.getAttribute("data-v");
          computeQuiz();
        });
      });
    });
    if (qGoBtn) qGoBtn.addEventListener("click", function () {
      var card = qSku && document.querySelector('.product[data-sku="' + qSku + '"]');
      if (!card) return;
      /* Dọn spotlight và timer cũ để bấm lại nhiều lần vẫn sáng đủ 3.2s */
      var prev = document.querySelector(".product--spotlight");
      if (prev) prev.classList.remove("product--spotlight");
      if (qSpotTimer) clearTimeout(qSpotTimer);
      card.scrollIntoView({ behavior: "smooth", block: "center" });
      card.classList.add("product--spotlight");
      qSpotTimer = setTimeout(function () {
        card.classList.remove("product--spotlight");
        qSpotTimer = null;
      }, 3200);
    });
    if (qResetBtn) qResetBtn.addEventListener("click", function () {
      qPicks = {}; qSku = "";
      qResult.hidden = true;
      quizBox.querySelectorAll(".quiz__opt").forEach(function (o) {
        o.classList.remove("is-picked");
        o.setAttribute("aria-pressed", "false");
      });
    });
